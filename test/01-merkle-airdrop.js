...
describe("Test merkle", async function () {
  ...
  describe("Interact with airdrop facet ", async function () {
    ...
    it("should add the first address airdrop correctly", async function () {
      ...
      airdropContract = await await ethers.getContractAt(
        "MerkleAirdropFacet",
        merkleDiamond
      );
    ...
    it("should add the first gotchi airdrop correctly and return that the gotchis have not claimed", async function () {
      await airdropContract.addGotchiAirdrop(
        "For Stani Fans",
        currentRoot2,
        diamondAddress,
        7,
    ...
    it("should mint two wearable items to the merkle distributor contract", async function () {
      ...
      describe("Interact with airdrop facet ", async function () {
        ...
        it("should add the first gotchi airdrop correctly and return that the gotchis have not claimed", async function () {
          ...
          const gotchiEv = await airdropContract.areGotchisClaimed(
            [3410, 6845],
            1
          );
          console.log(gotchiEv);
        ...
        it("should mint two wearable items to the merkle distributor contract", async function () {
          ...
          await daoMinterConnect.updateItemTypeMaxQuantity(
            itemsToMint,
            [100, 100]
          );
        ...
        it("should allow any address to claim wearables for eligible gotchis", async function () {
          ...
          let rec1sign = await ethers.getSigner(recipient1);
          airdropContract1 = await airdropContract.connect(rec1sign);
          const claimFor = await airdropContract1.claimForGotchis(
            1,
            [gotchi1, gotchi2],
            [gotchi1Object.itemId, gotchi2Object.itemId],
        ...
        it("balance should remain unchanged without reverting if gotchi(s) has been claimed before", async function () {
          const returns = await airdropContract1.claimForGotchis(
            1,
            [gotchi1, gotchi2],
            [gotchi1Object.itemId, gotchi2Object.itemId],
            [gotchi1Object.amountToClaim, gotchi2Object.amountToClaim],
          ...
          const itemEv = await airdropContract.areGotchisClaimed(
            [gotchi1, gotchi2],
            1
          );
